...
static int save_jpeg(const char *filename, const uint8_t *data, int width, int height, int channels, int quality);
...
#ifdef DEBUG
        // Save debug image
        save_jpeg("input.jpg", input, input_tensors->shapes[0][1], input_tensors->shapes[0][2], 3, 90);
#endif
...
static int save_jpeg(const char *filename, const uint8_t *data,
                     int width, int height, int channels, int quality)
{
    if (channels != 1 && channels != 3)
    {
        fprintf(stderr, "Only 1-channel (grayscale) or 3-channel (RGB) images are supported.\n");
        return 1;
    }
    ...
    {
        row_pointer = (JSAMPROW)&data[cinfo.next_scanline * row_stride];
